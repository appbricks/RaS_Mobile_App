...
export default class Avatar {
  ...
  updateAvatar(user) {
    ...
    if (user.profilePictureUrl) {
      ...
      if (emailAddress && emailAddress.length > 0) {
        ...
        gplusUrl = this._gplusProfilePicture(
          url => {
   ...
   * @param {*} email 
   */
  _gplusProfilePicture(cb, email) {

    const url = "https://picasaweb.google.com/data/entry/api/user/" + email + "?alt=json";
    this.logger.trace("retrieving gplus profile from", url);

    fetch(url)
      .then(response => response.json())
      .then(responseJSON => {

        this.logger.trace("gplus profile", responseJSON);
          ...
          fetch(photoUrl)
            .then(response => {
              ...
              if (contentLength && parseInt(contentLength) > 2506) {
                ...
                this.logger.trace("no gplus profile picture found");
                cb(null);
              }
            })
            .catch(error => {
              this.logger.trace("unable to retrieve gplus profile picture", error);
              cb(null);
            });
  ...
  _gravatar(cb, email, options) {
    ...
    fetch("https://www.gravatar.com/" + emailHash + ".json")
      .then(response => {
        this.logger.trace("gravatar response", response);
        if (response.ok) {
      ...
      .catch(error => {
        this.logger.trace("unable to retrieve gravatar user profile", error);
        cb(null);
      });
